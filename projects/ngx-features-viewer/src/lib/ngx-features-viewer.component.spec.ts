...
describe('NgxFeaturesViewerComponent Race Conditions & Temporal Dependencies', () => {
  ...
  it('should render identically for direct initialization vs runtime updates', () => {
    ...
    // 4. Compare both HTML outputs, ignoring generated UUIDs for clip-paths and masks
    const uuidRegex = /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/g;
    const normalizedUpdatedHtml = updatedHtml.replace(uuidRegex, 'MOCK-UUID');
    const normalizedDirectHtml = directHtml.replace(uuidRegex, 'MOCK-UUID');

    expect(normalizedUpdatedHtml).withContext('The updated SVG should exactly match the directly initialized SVG').toEqual(normalizedDirectHtml);
  });
});
