...
export class NgxFeaturesViewerComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  // Original scale, use this for initialization
  public get scale() {
    return this.zoomService.scale;
  }

  // Updated scale, use this for poltting features
  public get scaled() {
  ...
  constructor(
    ...
    const features$ = this.features$.pipe(
      ...
      tap((features) => {
        ...
        this.zoomService.scale.y.domain(domain);
      }),
      // Cache result
      shareReplay(1),
    );
    ...
    const resized$ = this.resizeService.resized$.pipe(
      ...
      tap(() => {
        ...
        const ticks = domain.length - 2;
        // Compute row size
        const inner = (outer - margin.top - margin.bottom) / ticks;
        // Define range
        let range: number[];
        // Set each domain accroging to inner height
        range = domain.map((_, i) => margin.top + inner * i + inner / 2);
        // Add initial margin on top
        range = [0, ...range, outer - this.margin.top - this.margin.bottom];
        // Update range in scale object
        this.scale.y.range(range);
      }),
    ...
    const zoomed$ = this.zoomService.zoomed$.pipe(
      ...
      tap((scale) => {
        ...
        this.axes.y.attr('transform', `translate(${margin.left}, 0)`);
        // Update vertical axis
        this.axes.y.call(d3.axisLeft(scale.y));
        // Set axis in correct position
        this.axes.x.attr('transform', `translate(0, ${height - margin.bottom})`);
        // Update horizontal axis
        this.axes.x.call(d3.axisBottom(scale.x));
      }),
    );
    ...
    this.update$ = this.initService.initialized$.pipe(
      ...
      // On sequence change, initialize scale
      ...
      switchMap(() => combineLatest([this.sequence$, this.features$])),
      ...
      tap(({ sequence }) => {
        ...
        const y = this.scaled.y('sequence');
