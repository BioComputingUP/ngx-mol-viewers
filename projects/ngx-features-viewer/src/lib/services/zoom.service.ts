  ...
  constructor(private initService: InitializeService) {
    ...
    this._brush = this.initService.initialized$.pipe(
      ...
      map((selection) => {
          ...
          if (!selection) {
            ...
            const k = (this.initService.sequence.sequence.length + 1) / (end - start);
    ...
    this.zoomed$ = scaled$.pipe(
      ...
      map(() => {
        ...
        if (end > this.initService.sequence.sequence.length + .5) {
          x.domain([start, this.initService.sequence.sequence.length + 0.5]);
        }
        ...
        const ticks = scale.x
          ...
          .filter((d) => Number.isInteger(d) && d >= 0.5 && d <= this.initService.sequence.sequence.length);
